...
struct z_curve_bottom : private contain<std::size_t>
{
	...
	using description =
		struct_description<char_pack<'z', '_', 'c', 'u', 'r', 'v', 'e', '_', 'b', 'o', 't', 't', 'o', 'm'>,
						   dims_impl<Dim>, dims_impl<>>;

	constexpr z_curve_bottom() = default;
	constexpr z_curve_bottom(std::size_t length) : base(length) {};

	constexpr auto construct() const { return z_curve_bottom<Dim>(); }

	static constexpr std::size_t size() { return 0; }
	constexpr std::size_t length() const { return base::template get<0>(); }
	static constexpr std::size_t offset(std::size_t i)
	{
		i = (i | (i << 8)) & 0x00FF00FF;
		i = (i | (i << 4)) & 0x0F0F0F0F;
		i = (i | (i << 2)) & 0x33333333;
...
struct z_curve_top : private contain<T, TH1, TH2>
{
	...
	using description = struct_description<char_pack<'z', '_', 'c', 'u', 'r', 'v', 'e', '_', 't', 'o', 'p'>,
										   dims_impl<>, dims_impl<>, type_param<T>, type_param<TH1>, type_param<TH2>>;

	template <typename... KS>
	using get_t = typename z_curve_top_get_t<T, KS...>::type;

	constexpr z_curve_top() = default;
	explicit constexpr z_curve_top(T sub_structure, TH1 sub_structure1, TH2 sub_structure2)
	...
	{
		return z_curve_top<decltype(this->base::template get<0>().construct(sub_structure)), TH3, TH4>(
	...
	constexpr std::size_t offset() const
	{
		return base::template get<0>().offset(
			(base::template get<1>().offset() | (base::template get<2>().offset() << 1)));
	}
};
